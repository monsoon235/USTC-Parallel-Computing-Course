...
std::tuple<double, double> calculate_pi_mpi(int max_iteration) {
...
int main(int argc, char *argv[]) {
    ...
    auto max_iteration = std::stoi(argv[1]);
    ...
    double pi, time;
    std::tie(pi, time) = calculate_pi_mpi(max_iteration);
    if (rank == 0) {
        std::cout << size << "\t" << max_iteration << '\t'
